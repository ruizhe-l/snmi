...
def class_to_str(mclass):
    if inspect.isfunction(mclass):
        return mclass.__name__
...
def config_to_str(config_dict):
    ...
    for k, v in config_dict.items():
        ...
        if isinstance(k, (numbers.Number, str)):
            if k.startswith('__'):
                continue
            s += f'{k}: '
        elif isinstance(k, dict):
            ss = config_to_str(k).replace('\n', '\n\t')
            s += f'{config_to_str(k)}:'
        else:
            s += f'{{{class_to_str(k)}}}\n:'
        ...
        if v is None:
            ...
            for l in v:
                ss = class_to_str(l).replace('\n', '\n\t')
                s += f'\n\t{{{ss}}}'
        elif isinstance(v, dict):
            ss = config_to_str(v).replace('\n', '\n\t')
            s += f'\n\t{ss}'
        else:
            ss = class_to_str(v).replace('\n', '\n\t')
            s += f'\n\t{{{ss}}}'
